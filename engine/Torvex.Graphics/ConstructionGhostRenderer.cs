...
public sealed unsafe class ConstructionGhostRenderer : IDisposable
{
    ...
    public ConstructionGhostRenderer(GL gl)
    {
        _gl = gl;
    }
    ...
    public void Render(
        ...
        float timeSeconds)
    {
        float pulse = 0.5f + MathF.Sin(timeSeconds * 4.5f) * 0.5f;
        float alpha = isValid
            ? 0.40f + pulse * 0.10f
            : 0.34f + pulse * 0.14f;

        Vector4 color = isValid
    ...
    private void CreateShader()
    {
        ...
        void main()
        {
            ...
            float light = 0.42 + max(dot(normal, lightDirection), 0.0) * 0.58;
            vec3 ghostColor = uColor.rgb * light;

            FragColor = vec4(ghostColor, uColor.a);
        }
        """;
        ...
        if (linkStatus == 0)
        {
            ...
            throw new InvalidOperationException($"Construction ghost shader link failed: {infoLog}");
        }
